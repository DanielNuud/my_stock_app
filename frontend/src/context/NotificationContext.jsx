...
export function NotificationsProvider({children}) {
  ...
  const [items, setItems] = useState([]);      // [{id,title,message,level,createdAt,readFlag}]
  const [open, setOpen] = useState(false);
  const lastSinceRef = useRef(null);           // ISO-строка, последний watermark
  ...
  useEffect(() => {
    ...
    load();
    const id = setInterval(load, 12000); // каждые 12с
